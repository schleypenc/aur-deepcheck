...
# Extended package list: generated by grepping the AUR git mirror directly.
# Covers 1500+ packages vs ~588 in the lenucksi list. Both are merged before
# pass 1 runs. Override with GRHT_URL='' to disable the remote fetch.
GRHT_URL="${GRHT_URL:-https://gr.ht/aur_pkg_list.txt}"
...
merge_package_lists() {
  # Merge the lenucksi list (~588) with the IRC/community grep list from gr.ht
  # which is generated directly from the AUR git mirror and covers 1500+ packages.
  # We extract package names from the refs/remotes/origin/<pkg> lines.
  local grht_url="$GRHT_URL"
  local merged="$COMMUNITY_DIR/package_list_merged.txt"
  local grht_tmp="$COMMUNITY_DIR/grht_raw.txt"

  if [ -z "$grht_url" ]; then
    info "GRHT_URL is empty - skipping extended package list fetch"
    return
  fi
  info "fetching extended package list from: $grht_url"
  if ! curl -fsSL --max-time 15 "$grht_url" -o "$grht_tmp" 2>/dev/null; then
    warn "could not fetch gr.ht package list - using lenucksi list only"
    return
  fi

  # Extract package names from lines like:
  #   commit abc123 (refs/remotes/origin/some-package-name)
  grep -oP '(?<=refs/remotes/origin/)[^)]+' "$grht_tmp" 2>/dev/null | sort -u > "${grht_tmp}.names" || true
  local grht_count
  grht_count=$(wc -l < "${grht_tmp}.names" 2>/dev/null || echo 0)

  # Merge with lenucksi list, deduplicate
  sort -u "$COMMUNITY_DIR/package_list.txt" "${grht_tmp}.names" > "$merged" 2>/dev/null || true
  local merged_count
  merged_count=$(wc -l < "$merged" 2>/dev/null || echo 0)

  if [ "$merged_count" -gt 0 ]; then
    export PACKAGE_LIST_FILE="$merged"
    info "package list: lenucksi($(wc -l < "$COMMUNITY_DIR/package_list.txt")) + gr.ht($grht_count) = $merged_count unique packages"
  else
    warn "merged list is empty - falling back to lenucksi list"
  fi

  rm -f "$grht_tmp" "${grht_tmp}.names"
}
...
run_pass2() {
  ...
  # Capture both sides of the pipe before re-enabling errexit.
  pipe_status=("${PIPESTATUS[@]}")
  set -e
  rc=${pipe_status[0]}
  [ "${pipe_status[1]:-0}" -eq 0 ] || err "tee failed writing pass 2 log: $log"
